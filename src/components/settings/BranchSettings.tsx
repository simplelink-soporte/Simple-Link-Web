...
export function BranchSettings() {
  ...
  const renderBranchCards = useMemo(() => {
    ...
    return branches.map((branch) => (
      <div
        key={branch.id}
        className="group relative py-3 px-4 border border-gray-200 rounded-lg bg-white hover:border-gray-300 transition-all duration-200"
      >
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex items-center gap-1.5 text-gray-600">
            <IconMapPin className="h-3.5 w-3.5 flex-shrink-0 text-gray-500" />
            <span className="truncate">{branch.address || 'Sin dirección'}</span>
          </div>
          <div className="flex items-center gap-1.5 text-gray-600">
            <IconPhone className="h-3.5 w-3.5 flex-shrink-0 text-gray-500" />
            <span className="truncate">{branch.phone || 'Sin teléfono'}</span>
          </div>
        </div>
        
        {branch.manager_id && (
          <div className="mt-2 flex items-center gap-1.5 text-xs text-gray-600">
            <IconUser className="h-3.5 w-3.5 flex-shrink-0 text-gray-500" />
            <span className="truncate">{branch.manager_id}</span>
          </div>
        )}
      </div>
    ))
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button 
          ...
          className="px-4 py-2 bg-white text-black border border-gray-200 rounded-md hover:bg-gray-50 hover:border-gray-300 flex items-center gap-2 text-sm shadow-sm transition-all duration-200"
        >
