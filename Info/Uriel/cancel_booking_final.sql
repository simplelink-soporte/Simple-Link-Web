-- Script completo para actualización de la función cancel_booking_v1
-- Para ser ejecutado en el editor SQL de Supabase

-- Primero, eliminar todas las versiones existentes de la función para evitar conflictos
DROP FUNCTION IF EXISTS public.cancel_booking_v1(uuid, text, boolean, numeric, text, text);
DROP FUNCTION IF EXISTS public.cancel_booking_v1(uuid, text, boolean, numeric, text, text, boolean, numeric, text);

-- Luego, crear la nueva versión con soporte para reembolsos
...
BEGIN
  ...
  -- 4. Actualizar la reserva con el estado de pago correcto
  UPDATE bookings
  SET 
    cancelled_at = NOW(),
    cancellation_reason = p_reason,
    payment_status = v_payment_status, -- Aquí se usa el estado correcto (refunded o cancelled)
    updated_at = NOW()
  WHERE id = p_booking_id;

  -- 5. Intentar registrar el reembolso en la tabla booking_refunds si existe y si hay reembolso
  IF p_has_refund THEN
    BEGIN
      -- Verificar si la tabla booking_refunds existe antes de intentar insertar
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'booking_refunds') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Ignorar error si no se puede insertar, pero registrar el error
      RAISE WARNING 'Error al registrar reembolso: %', SQLERRM;
    END;
  END IF;
...
Esta función actualiza la columna payment_status en la tabla bookings según corresponda.';

-- Para verificar que el tipo enum booking_payment_status incluye "refunded" 
-- Si no existe, es necesario agregarlo
DO $$
BEGIN
    -- Verificar si el valor "refunded" ya existe en el enum
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_enum e
        ...
        -- Agregar el valor "refunded" al enum
        ALTER TYPE booking_payment_status ADD VALUE IF NOT EXISTS 'refunded' AFTER 'cancelled';
    END IF;
END
$$;
