  ...
  useEffect(() => {
    if (isOpen && booking?.id && !stripeAccountId) {
      const loadOrganizationData = async () => {
        try {
          // Cargar conexión Stripe para obtener el accountId incluso en reservas sin garantía
          const connection = await loadStripeConnection();
          if (connection?.stripe_account_id) {
            console.log('✅ Stripe accountId obtenido para búsqueda de facturas:', connection.stripe_account_id);
            setStripeAccountId(connection.stripe_account_id);
          }
        } catch (error) {
          console.error('❌ Error al cargar datos organizacionales:', error);
        }
      };
